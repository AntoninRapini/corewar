...
** Last update Wed Apr  5 08:18:46 2017 romain pillot
*/
...
void		op_fork(t_vm *vm, t_process *process, t_param params[4])
{
  ...
  fork->data = data;
  fork->data_addr = (process->pc + params[0].value % IDX_MOD) % MEM_SIZE;
  fork->pc = fork->data_addr;
  fork->data_len = process->data_len;
  fork->cycle_cooldown = 0;
  fork->last_live_cycle = process->last_live_cycle;
  list_add(vm->processes, fork);
  i = -1;
  while(++i < process->data_len)
    vm->memory[(fork->data_addr + i) % MEM_SIZE] = fork->data[i];
}
